...
protected:
    ...
    IntVarArray yCoords;
public:
    //static member function size that we may use
    static int size() {
        return 1;
    }
    //number of squares
    ...
    Square(const SizeOptions& opt)
         ...
         xCoords(*this, n ,0, (n*(n-1)/2) - 1),
         yCoords(*this, n ,0, (n*(n-1)/2) - 1) {

        //number of squares
        const int n = opt.size();

        //Constraint for position of
        for (int i = 0; i < n; i++) {
            rel(*this, xCoords[i], IRT_LQ, (n*(n-1)/2) - (n-i));
            rel(*this, yCoords[i], IRT_LQ, (n*(n-1)/2) - (n-i));
        }
//TODO fix reificated propagators for coordinates and size described in #2 in instructions
        //Constraints for x coordinates
        for (int cx=0; cx<n; cx++) {
            BoolVarArgs bx(*this,n,0,1);
            for (int i=0; i<n; i++)
                //xCoords[i] must be less than cx-(n-i)+1 and bigger than cx
                dom(*this, xCoords[i], cx-(n-i)+1, cx, bx[i]);
            linear(*this, saz, bx, IRT_EQ, w);
        }
